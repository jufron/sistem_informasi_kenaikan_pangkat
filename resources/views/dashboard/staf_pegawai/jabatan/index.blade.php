<x-layouts.app title="Jabatan">
  ...
  <div class="section-body">
    <div class="card">
      <div class="card-body">
        <x-button-link href="{{ route('staf_pegawai.jabatan.create') }}" className="btn btn-primary my-4">
          Tambah
        </x-buttonLink>
        <x-table
          id="table_jabatan"
          ...
          <tr>
            ...
            <td>{{ $jbtn->nama }}</td>
            <td>{{ $jbtn->tanggal_buat }}</td>
            <td>
              <form id="form_delete_jabatan" action="{{ route('staf_pegawai.jabatan.delete', $jbtn) }}" method="POST">
                @method('delete')
                @csrf
  ...
  <x-slot:scriptOptional>
    ...
    <script>
      ...
      const modals_delete_jabatan = document.querySelectorAll('#form_delete_jabatan');
